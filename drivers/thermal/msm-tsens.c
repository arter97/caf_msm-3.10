...
struct tsens_tm_device_sensor {
	...
	struct tsens_thrshld_state	up_low_thresholds;
	int				dbg_adc_code;
};
			...
			int trip, enum thermal_trip_activation_mode mode)
{
	...
	case TSENS_TRIP_WARM:
		tmdev->sensor[tm_sensor->sensor_hw_num].
				up_low_thresholds.high_th_state = mode;
	...
	case TSENS_TRIP_COOL:
		tmdev->sensor[tm_sensor->sensor_hw_num].
				up_low_thresholds.low_th_state = mode;
				   ...
				   int trip, unsigned long temp)
{
	...
	case TSENS_TRIP_WARM:
		tmdev->sensor[tm_sensor->sensor_hw_num].
					up_low_thresholds.high_adc_code = code;
		tmdev->sensor[tm_sensor->sensor_hw_num].
					up_low_thresholds.high_temp = temp;
	...
	case TSENS_TRIP_COOL:
		tmdev->sensor[tm_sensor->sensor_hw_num].
					up_low_thresholds.low_adc_code = code;
		tmdev->sensor[tm_sensor->sensor_hw_num].
					up_low_thresholds.low_temp = temp;
...
static irqreturn_t tsens_irq_thread(int irq, void *data)
{
	...
	for (i = 0; i < tm->tsens_num_sensor; i++) {
		...
		if (status & TSENS_SN_STATUS_UPPER_STATUS) {
			...
			upper_thr = true;
			tmdev->sensor[i].up_low_thresholds.
