...
struct dm_req_dev {
	...
	struct workqueue_struct *req_crypt_split_io_queue;
	mempool_t *req_io_pool;
	mempool_t *req_page_pool;
	mempool_t *req_scatterlist_pool;
	bool is_fde_enabled;
	struct crypto_ablkcipher *tfm;
	unsigned int encryption_mode;
	struct ice_crypto_setting *ice_settings;
	unsigned int num_engines;
	unsigned int num_engines_fde;
	unsigned int fde_cursor;
	unsigned int num_engines_pfe;
	unsigned int pfe_cursor;
...
struct req_dm_crypt_io_wrapper {
	struct req_dm_crypt_io io_req;
	struct dm_req_dev *pdm_req;
};
...
static  bool req_crypt_should_encrypt(struct req_dm_crypt_io *req)
{
	...
	struct req_dm_crypt_io_wrapper *pwrapper =
		container_of(req, struct req_dm_crypt_io_wrapper, io_req);
	struct dm_req_dev *pdm_req = pwrapper->pdm_req;
...
static void req_crypt_dec_pending_encrypt(struct req_dm_crypt_io *io)
{
	...
	pwrapper = container_of(io, struct req_dm_crypt_io_wrapper, io_req);
	pdm_req = pwrapper->pdm_req;

	atomic_dec(&io->pending);

	if (error < 0) {
		dm_kill_unmapped_request(clone, error);
		mempool_free(pwrapper, pdm_req->req_io_pool);
	} else
...
static void req_crypt_dec_pending_decrypt(struct req_dm_crypt_io *io)
{
	...
	struct request *clone = NULL;
	struct req_dm_crypt_io_wrapper *pwrapper =
		container_of(io, struct req_dm_crypt_io_wrapper, io_req);
	struct dm_req_dev *pdm_req = pwrapper->pdm_req;
...
static void req_cryptd_crypt_read_convert(struct req_dm_crypt_io *io)
{
	...
	req_sg_read = (struct scatterlist *)mempool_alloc(
			pdm_req->req_scatterlist_pool, GFP_KERNEL);
	if (!req_sg_read) {
	...
	if ((clone->__data_len >= (MIN_CRYPTO_TRANSFER_SIZE *
			engine_list_total)) && (engine_list_total > 1))
		split_transfers = 1;

	if (split_transfers) {
		split_io = kcalloc(engine_list_total,
				sizeof(struct req_dm_split_req_io), GFP_KERNEL);
		if (!split_io) {
...
ablkcipher_req_alloc_failure:
	...
	mempool_free(req_sg_read, pdm_req->req_scatterlist_pool);
	kfree(split_io);
submit_request:
	if (io) {
		io->error = error;
...
static void req_cryptd_crypt_write_convert(struct req_dm_crypt_io *io)
{
	...
	req = ablkcipher_request_alloc(pdm_req->tfm, GFP_KERNEL);
	if (!req) {
	...
	crypto_ablkcipher_clear_flags(pdm_req->tfm, ~0);
	crypto_ablkcipher_setkey(pdm_req->tfm, NULL, KEY_SIZE_XTS);

	req_sg_in = (struct scatterlist *)mempool_alloc(
				pdm_req->req_scatterlist_pool, GFP_KERNEL);
	if (!req_sg_in) {
	...
	req_sg_out = (struct scatterlist *)mempool_alloc(
				pdm_req->req_scatterlist_pool, GFP_KERNEL);
	if (!req_sg_out) {
	...
	rq_for_each_segment(bvec, clone, iter) {
		if (bvec->bv_len > remaining_size) {
			page = NULL;
			while (page == NULL) {
				page = mempool_alloc(pdm_req->req_page_pool,
								gfp_mask);
				if (!page) {
...
ablkcipher_req_alloc_failure:
	...
	mempool_free(req_sg_in, pdm_req->req_scatterlist_pool);
	mempool_free(req_sg_out, pdm_req->req_scatterlist_pool);
submit_request:
	if (io) {
		io->error = error;
...
static void req_cryptd_split_req_queue(struct req_dm_split_req_io *io)
{
	...
	queue_work(io->pdm_req->req_crypt_split_io_queue, &io->work);
}
...
static void req_cryptd_queue_crypt(struct req_dm_crypt_io *io)
{
	...
	queue_work(pdm_req->req_crypt_queue, &io->work);
}
			    ...
			    int error, union map_info *map_context)
{
	...
	struct req_dm_crypt_io_wrapper *pwrapper =
		container_of(req_io, struct req_dm_crypt_io_wrapper, io_req);

	if (unlikely(!pdm_req)) {
		DMERR("%s pdm_req is NULL\n", __func__);
		return -EIO;
	}
	if (unlikely(pwrapper->pdm_req != pdm_req)) {
		DMERR("%s pdm_req %p is not equal %p\n", __func__,
			pdm_req, pwrapper->pdm_req);
		return -EIO;
	}
			 ...
			 union map_info *map_context)
{
	...
	struct bio *bio_src = NULL;
	struct req_dm_crypt_io_wrapper *pwrapper = NULL;

	if (unlikely(!pdm_req)) {
		DMERR("%s pdm_req is NULL\n", __func__);
		return -EIO;
	}
	pwrapper = mempool_alloc(pdm_req->req_io_pool, GFP_NOWAIT);
	if (!pwrapper) {
		DMERR("%s req_io allocation failed\n", __func__);
		error = DM_REQ_CRYPT_ERROR;
		goto submit_request;
	}
	req_io = &pwrapper->io_req;
	pwrapper->pdm_req = pdm_req;
	...
	clone->q = bdev_get_queue(pdm_req->dev->bdev);
	clone->rq_disk = pdm_req->dev->bdev->bd_disk;

	__rq_for_each_bio(bio_src, clone) {
		bio_src->bi_bdev = pdm_req->dev->bdev;
	...
	if (pdm_req->encryption_mode ==
		...
		memcpy(&req_io->ice_settings, pdm_req->ice_settings,
...
static void deconfigure_qcrypto(struct dm_req_dev *pdm_req)
{
	if (pdm_req->req_page_pool) {
		mempool_destroy(pdm_req->req_page_pool);
		pdm_req->req_page_pool = NULL;
	}

	if (pdm_req->req_scatterlist_pool) {
		mempool_destroy(pdm_req->req_scatterlist_pool);
		pdm_req->req_scatterlist_pool = NULL;
	}

	if (pdm_req->req_crypt_split_io_queue) {
		destroy_workqueue(pdm_req->req_crypt_split_io_queue);
		pdm_req->req_crypt_split_io_queue = NULL;
	}
	if (pdm_req->req_crypt_queue) {
		destroy_workqueue(pdm_req->req_crypt_queue);
		pdm_req->req_crypt_queue = NULL;
	}

	if (pdm_req->_req_dm_scatterlist_pool)
		kmem_cache_destroy(pdm_req->_req_dm_scatterlist_pool);

	mutex_lock(&pdm_req->engine_list_mutex);
	kfree(pdm_req->pfe_eng);
	pdm_req->pfe_eng = NULL;
	kfree(pdm_req->fde_eng);
	pdm_req->fde_eng = NULL;
	mutex_unlock(&pdm_req->engine_list_mutex);

	if (pdm_req->tfm) {
		crypto_free_ablkcipher(pdm_req->tfm);
		pdm_req->tfm = NULL;
	}
}
...
static void req_crypt_dtr(struct dm_target *ti)
{
	...
	if (pdm_req->req_io_pool) {
		mempool_destroy(pdm_req->req_io_pool);
		pdm_req->req_io_pool = NULL;
	}

	if (pdm_req->encryption_mode ==
				DM_REQ_CRYPT_ENCRYPTION_MODE_TRANSPARENT) {
		kfree(pdm_req->ice_settings);
		pdm_req->ice_settings = NULL;
	} else {
		deconfigure_qcrypto(pdm_req);
	}

	if (pdm_req->_req_crypt_io_pool)
		kmem_cache_destroy(pdm_req->_req_crypt_io_pool);

	if (pdm_req->dev) {
		dm_put_device(ti, pdm_req->dev);
		pdm_req->dev = NULL;
	}
...
static int configure_qcrypto(struct dm_req_dev *pdm_req)
{
	...
	pdm_req->tfm = crypto_alloc_ablkcipher("qcom-xts(aes)", 0, 0);
	if (IS_ERR(pdm_req->tfm)) {
	...
	pdm_req->num_engines_fde = pdm_req->num_engines_pfe = 0;

	mutex_lock(&pdm_req->engine_list_mutex);
	pdm_req->num_engines = (dm_qcrypto_func.get_num_engines)();
	if (!pdm_req->num_engines) {
	...
	eng_list = kcalloc(pdm_req->num_engines, sizeof(*eng_list), 0);
	if (NULL == eng_list) {
	...
	pdm_req->fde_eng = kcalloc(pdm_req->num_engines_fde,
				sizeof(*pdm_req->fde_eng), GFP_KERNEL);
	if (NULL ==  pdm_req->fde_eng) {
		DMERR("%s fde engine list allocation failed\n", __func__);
		mutex_unlock(&pdm_req->engine_list_mutex);
		goto exit_err;
	}

	pdm_req->pfe_eng = kcalloc(pdm_req->num_engines_pfe,
				sizeof(*pdm_req->pfe_eng), GFP_KERNEL);
	if (NULL == pdm_req->pfe_eng) {
		DMERR("%s pfe engine list allocation failed\n", __func__);
		mutex_unlock(&pdm_req->engine_list_mutex);
		goto exit_err;
	}

	pdm_req->fde_cursor = 0;
	...
	pdm_req->_req_dm_scatterlist_pool = kmem_cache_create(
		"req_dm_scatterlist", sizeof(struct scatterlist) * MAX_SG_LIST,
				 __alignof__(struct scatterlist), 0, NULL);
	if (!pdm_req->_req_dm_scatterlist_pool)
		goto exit_err;

	pdm_req->req_crypt_queue = alloc_workqueue("req_cryptd",
					WQ_UNBOUND |
					WQ_CPU_INTENSIVE |
					WQ_MEM_RECLAIM,
					0);
	...
	pdm_req->req_crypt_split_io_queue = alloc_workqueue("req_crypt_split",
					WQ_UNBOUND |
					WQ_CPU_INTENSIVE |
					WQ_MEM_RECLAIM,
					0);
	...
	pdm_req->req_scatterlist_pool = mempool_create_slab_pool(MIN_IOS,
					pdm_req->_req_dm_scatterlist_pool);
	if (!pdm_req->req_scatterlist_pool) {
	...
	pdm_req->req_page_pool = mempool_create_page_pool(MIN_POOL_PAGES, 0);
	if (!pdm_req->req_page_pool) {
...
static int req_crypt_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	ti->private = NULL;
	pdm_req = kzalloc(sizeof(*pdm_req), GFP_KERNEL);
	if (!pdm_req) {
		ti->error = "Cannot allocate encryption context";
		return -ENOMEM;
	}
	ti->private = pdm_req;
	mutex_init(&pdm_req->engine_list_mutex);
	...
	if (argv[3]) {
		if (dm_get_device(ti, argv[3],
				dm_table_get_mode(ti->table), &pdm_req->dev)) {
	...
	pdm_req->start_sector_orig = tmpll;

	/* Allow backward compatible */
	if (argc >= 6) {
		if (argv[5]) {
			if (!strcmp(argv[5], "fde_enabled"))
				pdm_req->is_fde_enabled = true;
			else
				pdm_req->is_fde_enabled = false;
		} else {
		...
		pdm_req->is_fde_enabled = true; /* backward compatible */
	}

	pdm_req->_req_crypt_io_pool = KMEM_CACHE(req_dm_crypt_io_wrapper,
						SLAB_HWCACHE_ALIGN);
	if (!pdm_req->_req_crypt_io_pool) {
		err =  DM_REQ_CRYPT_ERROR;
		goto ctr_exit;
	...
	if (pdm_req->encryption_mode ==
		...
		pdm_req->ice_settings =
			kzalloc(sizeof(struct ice_crypto_setting), GFP_KERNEL);
		if (!pdm_req->ice_settings) {
			err = -ENOMEM;
			goto ctr_exit;
		}
		pdm_req->ice_settings->key_size = ICE_CRYPTO_KEY_SIZE_128;
		pdm_req->ice_settings->algo_mode = ICE_CRYPTO_ALGO_MODE_AES_XTS;
		pdm_req->ice_settings->key_mode = ICE_CRYPTO_USE_LUT_SW_KEY;
		if (kstrtou16(argv[1], 0, &pdm_req->ice_settings->key_index) ||
			pdm_req->ice_settings->key_index < 0 ||
			pdm_req->ice_settings->key_index >
						MAX_MSM_ICE_KEY_LUT_SIZE) {
			DMERR("%s Err: key index %d received for ICE\n",
				__func__, pdm_req->ice_settings->key_index);
			err = DM_REQ_CRYPT_ERROR;
			goto ctr_exit;
		}
	} else {
		ret = configure_qcrypto(pdm_req);
		if (ret) {
				 ...
				 iterate_devices_callout_fn fn, void *data)
{
	...
	return fn(ti, pdm_req->dev, pdm_req->start_sector_orig, ti->len, data);
}
